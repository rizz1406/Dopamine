...
function gameCard({ href, emoji, name, desc, streakKey, glow, daily }) {
  const st = store.streak(streakKey);
  const hot = st.current > 0;
  return `
    <a class="game-card" href="${href}" data-nav style="--glow:${glow}">
